...
% Weighted open-loop with same weights as before
...
% Probably due to internal memory handling, one shouldn't measure the
% running times for all algorithms at once, but restart Matlab several
% times...
if isfile('AT.mat')
    AT = load('AT.mat').AT;
else
...
hold on; box on
set(gca, 'YScale', 'log')
plot(1:length(T), AT(1, :), 'x', 'LineWidth', 1.8, 'MarkerSize', 10)
plot(1:length(T), AT(2, :), 'x', 'LineWidth', 1.8, 'MarkerSize', 10)
plot(1:length(T), AT(3, :), 'x', 'LineWidth', 1.8, 'MarkerSize', 10)
plot(1:length(T), AT(4, :), 'x', 'LineWidth', 1.8, 'MarkerSize', 10)
plot(1:length(T), AT(5, :), 'o', 'LineWidth', 1.5, 'MarkerSize', 8)
plot(1:length(T), AT(6, :), 'd', 'LineWidth', 1.5, 'MarkerSize', 8)
xticks(1:length(T));
