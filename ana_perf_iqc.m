...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% This function deals with feedback interconnections of the form
%  x(t+1) = A  x(t) + B   w(t) + B2  d(t),  
%    z(t) = C  x(t) + D   w(t) + D12 d(t),     
%    e(t) = C2 x(t) + D21 w(t) + D22 d(t)
%    w(t) = Del(z)(t)
% and returns an upper bound on the corresponding robust energy gain.

% Here, Del is the impulsive operator defined by
...
% ----- Output ---------------------------------------------------------
%   gao      - Numerically determined upper bound on the system's robust 
%              energy gain.
function gao = ana_perf_iqc(sys, T, lI, opt)
    % Some sanity checks
    arguments
        sys        {mustBeA(sys, "ss")}
        T   (1, 2) {mustBeNonnegative}
        lI  (1, 1) {mustBeNonnegative, mustBeInteger}
        ...
        opt.psi {mustBeA(opt.psi, "ss")} = ...
                  basis_filter(6, 2*lI, sampling_time=sys.Ts, ...
                               pole=0, type=2)
    ...
    % Dimensions of input and output channels
    inp = [lI, size(sys, 2) - lI];
    out = [lI, size(sys, 1) - lI];
    ...
    % For the system LMI
    sye      = [eye(size(sys, 2)); sys];
    [pnl, ~] = outerfactor_permutation(inp, out, 0); 
    sye      = blkdiag(ps, eye(dis+err)) * sye(pnl, :); % Filtered system
    OX       = factor(sye);
    ...
    % Upper bound on robust energy gain
    [~, ~, sga] = lmivar(1, [1, 1]);
    ...
    IXr = blkdiag(zeros(2*lxf+lmI), eye(err), zeros(dis));
    ...
    function add_multiplier_constraints(Z, sZ, M, sM)
        e = eye(lI);     % An abbreviation
        z = zeros(lI);   % Another abbreviation
